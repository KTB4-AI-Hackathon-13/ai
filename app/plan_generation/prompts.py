...
사용자가 이번 턴에 그 계획에 대한 피드백(user_message)을 남겼다. **이번 턴에 새로
반영해야 할 지시는 user_message 하나뿐이다.** 이 피드백을 최우선으로, 정확히 한 번만
반영해 계획을 수정한다.

입력에는 feedback_history(지난 턴들에서 사용자가 남겼던 피드백 원문 목록)도 함께
주어지는데, 이건 대화 맥락을 참고하라고 주는 기록일 뿐이다. feedback_history에 있는
과거 항목들은 이미 current_plan에 전부 반영되어 있는 상태다 — 그 항목들을 새로 실행할
지시로 다시 읽거나, user_message와 합쳐서 두 번 반영하지 않는다. current_plan을
"지금 상태"의 유일한 근거로 삼고, feedback_history는 왜 지금 이 모습이 됐는지 배경으로만
참고한다.
...
1. user_message에서 요청한 변경 사항(기간 조정, 특정 날짜 제외, 강도 조절 등)을 최우선으로,
   딱 한 번만 반영한다. 요청과 무관한 부분은 가능한 한 current_plan을 그대로 유지해
   불필요한 변경을 최소화한다.
2. 각 과제는 scheduled_date(YYYY-MM-DD)로 표시하고, 반드시 template_answers.start_date ~
   ...
   새로 만들어 넣지 않는다. "N일 연장/단축"처럼 상대적인 기간 조정 요청은
   template_answers가 아니라 current_plan에 실제로 들어있는 날짜의 최솟값/최댓값을
   기준으로 계산한다 — template_answers는 최초 계획을 만들 때의 값이라 그 뒤에 있었던
   조정이 반영되지 않았을 수 있다.
...
8. "N단계로만", "N개만", "딱 N번만"처럼 절대적인 개수/횟수를 지정하는 요청은, 지금
   current_plan에 몇 개가 있는지와 상관없이 그 숫자를 최종 목표값으로 삼아 정확히 그
   개수가 되도록 만든다(더하거나 빼는 상대값으로 착각하지 않는다). "N일/N개 더
   늘려줘/줄여줘"처럼 상대적인 표현일 때만 현재 current_plan 기준으로 더하거나 뺀다.
9. 이번 user_message가 이전 assistant_message에서 이미 "반영했다"고 말한 내용과
   같아 보이거나, 사용자가 짧게 재확인("네", "응", "좋아요")하는 말이라면 — 그건
   current_plan을 다시 바꾸라는 새 지시가 아니라 직전 제안에 대한 반응이다. current_plan을
   또 변경하지 말고, 규칙 6에 따라 승인/확정 의도인지 아닌지만 판단한다.
"""